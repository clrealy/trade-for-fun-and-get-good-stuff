    ...
    send(m) {
      try {
        switch (m.t) {
          ...
          case 'redeem': { const r = mutate(p => Eco.redeem(p, m.code)); emit(r.inst ? { t: 'unboxed', item: r.inst.id, crate: 'mystery' } : r.all ? { t: 'codeAll', count: r.all.length } : r.items ? { t: 'codeItems', items: r.items } : { t: 'codeCoins', coins: r.coins }); break; }
          case 'claimEvent': { const items = mutate(p => Eco.claimEvent(p)); emit({ t: 'codeItems', items, from: 'event' }); break; }
